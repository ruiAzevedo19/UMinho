...
public class Loja implements Comparable<Loja>, Serializable {
    ...
    public Loja fromArgs(String[] args) {
        if (args.length != 4)
            return null;
        String codigoArgs = args[0].split(":")[1];
        String nomeArgs   = args[1];
        GPS    localArgs  = new GPS(Double.parseDouble(args[2]), Double.parseDouble(args[3]));
        return new Loja(codigoArgs, nomeArgs, localArgs, this.defaultprodutosDisponiveis());
    }
    ...
    private Set<LinhaEncomenda> defaultprodutosDisponiveis() {
        produtosDisponiveis = new TreeSet<>();
    ...
    @Override
    public int hashCode() {
        return Objects.hash(codigo, nome, local, produtosDisponiveis, encomendaQueue, porEntregar);
    }
