...
void drawVertices(void) {
	...
	for (int i = 0; i < allVertices.size(); i++) {
	    for(int j = 0; j < allVertices.at(i).size(); j++){
            glVertex3f(allVertices.at(i).at(j)->getX(),
                       allVertices.at(i).at(j)->getY(),
                       allVertices.at(i).at(j)->getZ());
        }
	}
...
void drawVerticesOp(void){
    ...
    glGenBuffers(allVertices.size(), buffers);
	for (int i = 0; i < allVertices.size(); i++) {
	    if(show.at(i)){
            for(int j = 0; j < allVertices.at(i).size(); j++){
                vertices.push_back(allVertices.at(i).at(j)->getX());
                vertices.push_back(allVertices.at(i).at(j)->getY());
                vertices.push_back(allVertices.at(i).at(j)->getZ());
            }
            float* arr = &vertices[0];
            glBindBuffer(GL_ARRAY_BUFFER, buffers[i]);
            glBufferData(GL_ARRAY_BUFFER, sizes.at(i)*3*sizeof(float), &arr[0], GL_STATIC_DRAW);
...
void drawCylinder(float radius, float height, int slices) {
	...
	for(i = 0; i < slices; i++){
		...
		glVertex3f(radius * sin(i * angle), height, radius * cos(i * angle));
		glVertex3f(radius * sin((i+1) * angle), height, radius * cos((i+1)* angle));

		glVertex3f(radius * sin(i * angle), height, radius * cos(i * angle));
		glVertex3f(radius * sin(i * angle), 0, radius * cos(i * angle));
		glVertex3f(radius * sin((i+1) * angle), height, radius * cos((i+1)* angle));

		glVertex3f(radius * sin((i+1) * angle), height, radius * cos((i+1)* angle));
		glVertex3f(radius * sin(i * angle), 0, radius * cos(i * angle));
		glVertex3f(radius * sin((i+1) * angle), 0, radius * cos((i+1)* angle));
...
int main(int argc, char **argv) {
    allVertices = readXML("mix.xml", PATH_TO_DB, &sizes);
    buffers = (GLuint*) malloc(sizeof(GLuint) * allVertices.size());
    for(int i = 0; i < allVertices.size(); i++){
        show.push_back(1);
    }
	int menu = 0;
