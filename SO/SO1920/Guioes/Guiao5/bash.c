...
char** getPipeCommand(char** argv, int* argc){
    int size = 20, i;
...
void executePipe(char** argv, int *argc){
    pid_t pid;
    int fd[2], end = 0;
    char** command;
    ...
    if( !pid ){
        while( !end ){
            if( !*argc )
                break;
            command = getPipeCommand(argv, argc);
            pipe(fd);
            dup2(fd[WRITE],1);
            close(fd[READ]);
            close(fd[WRITE]);
...
int main(int argc, char** argv){
    ...
    while( 1 ){
        ...
        ( findSpecialCase(argv,argc,"|") > 0 ) ? executePipe(argv, &nr_words) : execute(ws,&nr_words,background);
